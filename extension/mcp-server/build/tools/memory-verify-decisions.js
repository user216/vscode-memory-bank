...
export function registerMemoryVerifyDecisions(server) {
    server.tool("memory_verify_decisions", "Run compliance assertions embedded in accepted ADR decisions. Checks that the codebase matches what the ADRs prescribe. Each ADR can have a ## Verification section with ASSERT_FILE_EXISTS, ASSERT_FILE_NOT_EXISTS, ASSERT_CONTAINS, ASSERT_NOT_CONTAINS assertions as HTML comments.", {
        adr_id: z
            .string()
            .optional()
            .describe("ADR ID to verify (e.g. 'ADR-0015'). Omit to verify all accepted ADRs with assertions."),
    }, async ({ adr_id }) => {
...
//# sourceMappingURL=memory-verify-decisions.js.map