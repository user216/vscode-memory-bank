...
export function registerMemoryMigrateV1(server) {
    server.tool("memory_migrate_v1", "Detect and optionally migrate v1 layout artifacts (tasks/ and decisions/ subdirectories) to the flat v2 layout. Moves files to the memory-bank root, adds YAML frontmatter if missing, removes empty subdirectories. Use dry_run=true (default) to preview changes first.", {
        dry_run: z
            .boolean()
            .optional()
            .default(true)
