...
function updateFileStatus(content, status, today) {
    let result = content;
...
export function registerMemoryBulkUpdateStatus(server) {
    server.tool("memory_bulk_update_status", "Update the status of multiple tasks or decisions in a single call. Each item is validated independently; partial success is reported. Accepts common aliases (Open→Pending, Done→Completed, Draft→Proposed, Approved→Accepted).", {
        updates: z
            .array(z.object({
        ...
        for (const update of updates) {
            ...
            if (item.type === "task" && !TASK_STATUSES.includes(status)) {
                failures.push(`${update.id}: "${status}" is not valid for tasks`);
                continue;
            }
            if (item.type === "decision" && !DECISION_STATUSES.includes(status)) {
...
//# sourceMappingURL=memory-bulk-update-status.js.map