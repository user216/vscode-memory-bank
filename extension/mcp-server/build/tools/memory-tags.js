...
export function registerMemoryTags(server) {
    server.tool("memory_tags", "List all tags with item counts, or list all items with a specific tag. Tags come from YAML frontmatter and inline #hashtags.", {
        tag: z
            .string()
            .optional()
        ...
        if (tag) {
            ...
            if (!itemIds || itemIds.size === 0) {
                return {
                    content: [{ type: "text", text: `No items found with tag #${tag}` }],
                };
            }
        ...
        if (store.tags.size === 0) {
            return {
                content: [{ type: "text", text: "No tags found. Add tags via YAML frontmatter or inline #hashtags." }],
            };
        }
