...
interface AdrSections {
  ...
  alternatives: string;
  consequences: string;
  rest: string; // any trailing sections like Progress Log
}

function parseAdr(content: string): AdrSections | null {
...
function rebuildAdr(id: string, sections: AdrSections): string {
