...
export function syncAllFiles(memoryBankPath: string): number {
  const db = getDb();
...
export function syncSingleFile(
...
function syncCrossRefsForItem(parsed: ParsedItem): void {
  const db = getDb();
  const now = new Date().toISOString();

  const existingIds = new Set(
    (db.prepare("SELECT id FROM items").all() as Array<{ id: string }>).map(
      (r) => r.id,
    ),
  );
  ...
  db.prepare(
    "DELETE FROM links WHERE source_id = ? AND relation = 'references'",
  ).run(parsed.id);
...
export function watchMemoryBank(memoryBankPath: string): fs.FSWatcher {
